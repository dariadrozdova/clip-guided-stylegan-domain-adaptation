...
В рамках проекта был реализован и исследован подход CLIP-направляемой доменной адаптации предобученного генератора StyleGAN-2 без использования изображений целевого домена. В качестве целевого домена был выбран стиль *sketch*, что позволило наглядно проанализировать влияние различных уровней генератора на визуальную стилизацию.

В ходе экспериментов было показано, что обучение только поздних слоёв генератора приводит к появлению начальных признаков целевого стиля при сохранении фотореалистичной структуры лица. При расширении числа обучаемых слоёв эффект стилизации усиливается и становится более выраженным, что подтверждает важность средних и поздних уровней представления для доменной адаптации.

Для проверки была проведена генерация изображений для латентных векторов, не использовавшихся в процессе обучения. Сравнение выходов замороженной и обучаемой копий генератора показало, что адаптация носит устойчивый характер и не сводится к запоминанию отдельных примеров.

В дальнейшем работу можно расширить за счёт редактирования реальных изображений посредством инверсии в латентное пространство StyleGAN, а также за счёт более детального исследования функций потерь и стратегий обучения. Тем не менее, в рамках данного проекта удалось воспроизвести ключевые идеи метода StyleGAN-NADA, провести исследование важности блоков генератора и получить качественные результаты доменной адаптации.
